---
title: Deepfont on Keras
date: 2020-11-01
tags:
  - Machine Learning
---

Deepfont was first introduced by Adobe, which uses deep learning to identify font type. Inspired by their works, I made this reproduction using [Keras](https://keras.io).
...
- **AdobeVFR Dataset** A large set of labeled real-world images as well as a large corpus of unlabeled real-world data are collected for both training and testing, which could be found at the link [ Adobe Visual Font Recognition (VFR)](https://github.com/tensorflow/datasets/issues/431)
...
It not convenient to do the additional steps for each characters, so loosely speaking, we could done this before legacy steps, at the beginning we generate our datasets using _TextRecognitionDataGenerator_.

```shell
python3 run.py -c 10 -k 15 -rk -d 3 -do 2 -f 64 -ft ['Font1', 'Font2', 'Font3'] -t $(grep -c ^processor /proc/cpuinfo)
```

This generate 10 examples with _Font1_, _Font2_ and _Font3_ which characters sized 64x64 with a skewing angle between -15 and 15 and a random distorsions both vertical and horizontal, multi-threads acceleration enabled.
...
Here we use the _Text Recognition Data Generator_ CLI `trdg` to generate the random datasets.
...
Import pre-generated synthetic and realistic text images from `datasets_path` _(here especially the datasets we generated before)_.
